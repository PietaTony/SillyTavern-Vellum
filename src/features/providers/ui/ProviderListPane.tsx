...
import { byUsefulness } from '../popularity';
import { fetchProviderRows, setActiveProvider } from '../registryApi';
import { verifyProvider } from '../switchActive';
import { ProviderListRow } from './ProviderListRow';

/**
 * AI 供應商清單的**內容本體**（派工⑤ 優先序 2）。
 *
 * 🔴 **抽出來的理由是 SSOT，不是整潔。** 這一份現在有兩個入口：
 * 設定分頁 `/settings/providers` 與**對話頁 ☰ 的全螢層**（Peter 2026-08-26：
 * 「這個選項顯示跟 /settings 完全相同」）。留在路由檔裡就會被複製成兩份，
 * 然後其中一份慢慢長歪 —— 而使用者要學兩次。
 *
 ...
 */
export function ProviderListPane({ onOpen }: { onOpen: (id: string) => void }) {
