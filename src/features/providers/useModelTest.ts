 ...
 */
export function useModelTest(provider: string, onNotify: (m: ToastMsg) => void) {
  ...
  return useMutation({
    ...
    onSuccess: (r) => {
      onNotify(
        r.ok
          ? { severity: 'success', text: `測試成功，已存：${r.model}` }
          : { severity: 'warning', text: `測試沒有通過，沒有存：${r.message.slice(0, 120)}` },
      );
