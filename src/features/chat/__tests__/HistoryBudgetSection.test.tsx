...
import { HistoryBudgetSection } from '../ui/HistoryBudgetSection';
 ...
 *
 * 🔴 **原本測的是 `HistoryBudgetLayer.tsx`（含外層 `FullScreenLayer`），B5 把它
 * 抽成 `HistoryBudgetSection.tsx`（內容本身）＋ `LengthLimitsLayer.tsx`（外層＋
 * 跟 `MaxResponseSection` 併排）——這支測純內容，外層 reachability 的測試在
 * `LengthLimitsLayer.test.tsx`。**
 *
...
const openSection = async () => {
  render(<HistoryBudgetSection />);
  await screen.findByLabelText('對話歷史上限（bytes）');
};

describe('HistoryBudgetSection：四段說明文案逐字守衛（贏過 ST「不清不楚」的地方）', () => {
