 ...
 *
 * 🔴 **`data/` 就是全部的家當**（角色、對話、世界書、金鑰），位置是 `cwd/data`。
 * **備份＝複製這個資料夾**；更新版本＝把舊的 `data/` 整個搬進新的解壓目錄。
 *
 * 🔴 **這一行存在的理由是「空目錄看起來跟正常的空狀態一模一樣」**：
 * 啟動時 cwd 不對（例如雙擊啟動檔而 cwd 是家目錄），app 會一如往常地起來、
 * 顯示「還沒有好友」—— 看起來像新手畫面，實際上是舊資料還在別的地方。
 * 有這一行，重啟後看到「角色 0」就知道路徑跑掉了，而不是幾天後才發現。
 */
