...
| **X2 · Generated** | `src/app/routeTree.gen.ts` | Produced by the `tanstackRouter()` Vite plugin configured in `vite.config.ts` — there is no separate CLI. **It is in `.gitignore` — not version-controlled.** | Never hand-edit. **New worktree:** run `pnpm setup:worktree` — it wraps `pnpm install`, regenerates this file via `vite build`, then checks `tsc --noEmit` itself and exits non-zero (as its own bug, not yours) if it can't get you to a green typecheck. **Existing tree, this file looks wrong:** `pnpm dev`, `pnpm build`, and `pnpm verify` all regenerate it too, as a side effect of the `vite build` step each of them runs first. |
| **X3 · Assembly line & global settings** | `server/app.ts` `server/index.ts` `server/static.ts`<br>`server/lib/ids.ts` `server/lib/settingsModel.ts` `server/lib/settingsLimits.ts`<br>`server/services/settings.ts`<br>`src/app/AppProviders.tsx` `src/app/router.ts` `src/app/queryClient.ts` `src/app/setup.ts`<br>`src/main.tsx` | Four or more domains import these. `app.ts` mounts every route; `settings.ts` is read by H1, H2, H3 and H4. A change here is a change to everyone. 🔴 `settingsLimits.ts` (2026-08-31): `settingsModel.ts` hit `gate:file-size`'s 150-line cap; the "size/budget" keys (`historyByteBudget`, `maxOutputTokens`) were extracted here and merged back into `Settings` via an intersection type — declared by the B5-consolidation ticket, not a separate one. | **Open a task ticket.** Registering a new route counts. |
