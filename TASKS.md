...
## 🔴 下一輪第一件：等 Peter 在真手機上驗兩件（2026-08-27 收工）
1. **面板上三個數字有沒有變成 17 / 82 / 23** ── `d453cfed`（何思年(2)）的
   `chat.variables.stat_data` 已經有值了，重新整理就該看得到。
   還是三個 `—` 的話，代表卡片讀的不只是 `stat_data`，要再挖。
2. **點狀態卡的標題列（🐍 何思年）展不展得開** ── 我這邊的自動化**整個 session
   沒有任何一次點擊被證明進過 iframe**，而主頁的按鈕全都點得動。
   展得開 ⇒ 是我的工具，結案；展不開 ⇒ 是產品的命中測試問題。

## 還沒解的兩件（都要 Peter 決定）
- **「每一則回覆都有即時狀態欄」**：靠 `outputRules[9]`，觸發條件是訊息裡有
  `<StatusPlaceHolderImpl/>` ── **沒有任何人放它**（整份角色 JSON 與世界書掃過，
  只有規則 8/9 的 `find` 欄位有這個字）。ST 那邊是 MVU 用提示詞注入放的。
  兩條路：① 我們在提示詞裡要求模型每輪輸出它 ② 落地時我們自己補上（比較確定，
  但那是我們動了模型的輸出）。
